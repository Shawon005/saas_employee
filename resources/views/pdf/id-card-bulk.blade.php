...
<head>
    ...
    <style>
        ...
        .sheet { width: 100%; }
        .worker-pair { margin-bottom: 8mm; page-break-inside: avoid; text-align: center; }
        .card {
            position: relative;
            width: 85mm;
            height: 54mm;
...
<body>
    <div class="sheet">
        @foreach ($workers as $worker)
            <div class="worker-pair">
                @include('pdf.partials.id-card-pair', ['worker' => $worker, 'qrImage' => $qrService->generateQrDataUri($worker)])
            </div>
        @endforeach
    </div>
</body>
